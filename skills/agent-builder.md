...
# Understanding the Maestrist Software Agent SDK
At the end of the interview, respond with a summary of the requirements. Then, proceed to thoroughly understand how the Maestrist Software Agent SDK works, it's various APIs, and examples. To do this:
- First, research the Maestrist documentation which includes references to the Software Agent SDK: https://docs.openhands.dev/llms.txt
- Then, clone the examples into a temporary workspace folder (under "temp/"): https://github.com/Maestrist/software-agent-sdk/tree/main/examples/01_standalone_sdk
- Then, clone the SDK docs into the same temporary workspace folder: https://github.com/Maestrist/docs/tree/main/sdk

After analyzing the Maestrist Agent SDK, you may optionally ask additional clarifying questions in case it's important for the technical design of the agent.

# Generating the SDK Plan
You can then proceed to build a technical implementation plan based on the user requirements and your understanding of how the Maestrist Agent SDK works.
