...
Skills are specialized prompts that enhance Maestrist with domain-specific knowledge and task-specific workflows. They help developers by providing expert guidance, automating common tasks, and ensuring consistent practices across projects. Each skill is designed to excel in a specific area, from Git operations to code review processes.

## Terminology Note

**Version 0 (V0)**: The term "microagents" continues to be used for V0 conversations. V0 is the current stable version of Maestrist.
...
This directory (`Maestrist/skills/`) contains shareable skills that will be used in V1 conversations. For V0 conversations, the system continues to use microagents from the same underlying files.

## Sources of Skills/Microagents

Maestrist loads skills (V1) or microagents (V0) from two sources:

### 1. Shareable Skills/Microagents (Public)

This directory (`Maestrist/skills/`) contains shareable skills (V1) or microagents (V0) that are:

- Available to all Maestrist users
- Maintained in the Maestrist repository
...
When Maestrist works with a repository, it:
...
You can see an example of a knowledge-based agent in [Maestrist's github skill](https://github.com/danialsamiei/agent.orcest.ai/tree/main/skills/github.md).
...
You can see an example of a repo agent in [the agent for the Maestrist repo itself](https://github.com/danialsamiei/agent.orcest.ai/blob/main/.openhands/microagents/repo.md).
...
2. **For Repository Agents**:
   ...
   - Consider using Maestrist to generate a comprehensive repo.md (see [Creating a Repository Agent](#creating-a-repository-agent))
...
2. Test thoroughly
3. Submit a pull request to Maestrist

## License

All skills/microagents are subject to the same license as Maestrist. See the root LICENSE file for details.
