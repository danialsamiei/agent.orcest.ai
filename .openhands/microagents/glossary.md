...
### Agent
The core AI entity in Maestrist that can perform software development tasks by interacting with tools, browsing the web, and modifying code.
...
#### CodeAct Agent
[A generalist agent in Maestrist](https://arxiv.org/abs/2407.16741) designed to perform tasks by editing and executing code.
...
Settings that control various aspects of Maestrist behavior, including runtime, security, and agent settings.
...
### Interface
Different ways to interact with Maestrist.

#### CLI Mode
A command-line interface mode for interacting with Maestrist agents without a graphical interface.

#### GUI Mode
A graphical user interface mode for interacting with Maestrist agents through a web interface.

#### Headless Mode
A mode of operation where Maestrist runs without a user interface, suitable for automation and scripting.
...
### Microagent
A specialized prompt that enhances Maestrist with domain-specific knowledge, repository-specific context, and task-specific workflows.

#### Microagent Registry
A central repository of available microagents and their configurations.

#### Public Microagent
A general-purpose microagent available to all Maestrist users, triggered by specific keywords. Located in `microagents/`.
